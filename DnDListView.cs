...
namespace DnDListViewSpace
{
  public class DnDListView : System.Windows.Forms.ListView
  {
    #region Constants
    ...
    /// </summary>
    [Category("Drag Drop")]
    public event EventHandler<DnDListViewItemDragEventArgs> ItemDragDrop;
      ...
      if (this.IsRowDragInProgress)
      {
        try
        {
          ...
          if (dropItem != null)
          {
            ...
            {
              DnDListViewItemDragEventArgs args;
              Point clientPoint;

              clientPoint = this.PointToClient(new Point(drgevent.X, drgevent.Y));
              args = new DnDListViewItemDragEventArgs(dragItem, dropItem, dropIndex, this.InsertionMode, clientPoint.X, clientPoint.Y);
    ...
    /// </summary>
    /// <param name="e">The <see cref="DnDListViewItemDragEventArgs" /> instance containing the event data.</param>
    protected virtual void OnItemDragDrop(DnDListViewItemDragEventArgs e)
    {
      EventHandler<DnDListViewItemDragEventArgs> handler;
